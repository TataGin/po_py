...
def prompt_if_exists(path):
    ...
    if type(path) == str:
        path = pl.Path([path][0])
    if path.is_file():
        choice = custom_question('File already exists. What should we do',
                                 {'Cancel': 'c', 'Owerwrite': 'o',
                                  'Increment': 'i', 'Change': 'c'})
...
def uniquify(path, sep='_'):
    ...
    if s:
        return folder.joinpath(name[:s.start()] + str(int(name[s.start()+1:])
                                                      + 1) + suffix)
    else:
        return folder.joinpath(name + sep + '01' + suffix)
